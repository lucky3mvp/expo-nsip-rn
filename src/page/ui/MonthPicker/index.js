...
import { MonthPicker, List, ListItem, Popup } from 'component'
		...
		return (
			<View style={{flex: 1, backgroundColor: '#efefef'}}>
				<ScrollView style={{flex: 1, backgroundColor: '#efefef'}}>
					<List>
						<ListItem title={'默认选中最后一个。您选择: '+this.state.MonthPicker1Label} onPress={()=>{
							Popup.show(
								<MonthPicker
						...
						<ListItem title={'默认选中2017年3月。您选择: '+this.state.MonthPicker2Label} onPress={()=>{
							Popup.show(
								<MonthPicker
						...
						<ListItem title={'使用自行定义的数据,类似于普通picker了。您选择: '+this.state.MonthPicker3Label} onPress={()=>{
							Popup.show(
								<MonthPicker
